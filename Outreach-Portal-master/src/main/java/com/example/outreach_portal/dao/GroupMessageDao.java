...
public interface GroupMessageDao extends JpaRepository<GroupMessage,Integer> {
	...
	@Query("Select g from GroupMessage g where g.group=:group order by g.timestamp desc")
	List<GroupMessage> getGroupMessage(@Param("group") Group group);
	...
	@Transactional
	@Modifying
	@Query("Delete from GroupMessage m where m.user1 =:user1 and m.group = :group ")
	int leaveGroup(@Param("user1") User user1,@Param("group") Group group);
}
